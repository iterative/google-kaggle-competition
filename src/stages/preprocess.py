...
def main(cli_params):
    ...
    dataset.add_dir(data_root /params["data"]["training"] / dataset_name, dataset_type=fo.types.dataset_types.FiftyOneImageClassificationDataset)
    view_train,view_val,view_test = fo_random.random_split(dataset, [0.6,0.2,0.2], seed=51)

    
    export_dataset(view_train, str(data_root /params["data"]["training"] / dataset_name /'train'))
    export_dataset(view_val, str(data_root /  params["data"]["training"] /  dataset_name / "val"))
    export_dataset(view_test, str(data_root / params["data"]["training"] /  dataset_name / "test"))
